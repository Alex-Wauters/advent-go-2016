...
func part1(input string) {
	...
	fmt.Println(len(decompress(input)))
}

type unpack struct {
	text                 bytes.Buffer
	length, repeat       int
	inCommand, lengthSet bool
}
			...
			if u.isUnpacking() {
				...
				if !u.isUnpacking() {
					for i := 0; i < u.repeat; i++ {
						uncompressed.WriteString(u.text.String())
					}
					u = unpack{}
				}
			} else {
				uncompressed.WriteRune(c)
			}
		}
	}
...
func (u *unpack) setDigit(c rune) {
	if !u.lengthSet {
		if u.length > 0 {
			u.length = sToInt(fmt.Sprintf("%v%v", u.length, string(c)))
		} else {
			u.length = toInt(c)
		}
	} else {
		if u.repeat > 0 {
			u.repeat = sToInt(fmt.Sprintf("%v%v", u.repeat, string(c)))
		} else {
			u.repeat = toInt(c)
		}
	}
...
func (u *unpack) isUnpacking() bool {
	return u.text.Len() < u.length
}
...
func toInt(c rune) int {
	return sToInt(string(c))
}

func sToInt(s string) (i int) {
	i, _ = strconv.Atoi(s)
	return
}
